--[[
   Lifted & modified from Interface/AddOns/Blizzard_SharedXML/TreeListDataProvider.lua
   
   This is a relatively simple extension of LinearizedTreeDataProviderMixin that adds capacity
   to filter nodes out of the linearized view. TBH I'm somewhat surprised it wasn't already
   possible, but I suppose the need just hasn't arisen yet for blizzard.

   ... well it *would* be a simple extension anyways, if it weren't for the fact that
   for some godforsaken reason, TreeNodeMixin isn't a global in the original, so extending it
   in a normal way is impossible.

   So instnead, we'll just do a bit of copyright infringement and hope nobody cares :-)
--]]
...
local TreeDataProviderConstants = {
   Collapsed = true,
   Uncollapsed = false,
...
---@param node TreeNode
---@param skipInvalidation boolean?
...
---@class TreeDataProvider : CallbackRegistryMixin
---@field node TreeNode
local TreeDataProviderMixin = CreateFromMixins(CallbackRegistryMixin);
...
---@param collapsed boolean
---@param predicate fun(node:TreeNode):boolean
...
---@param indexBegin integer?
---@param indexEnd integer?
---@param excludeCollapsed boolean?
...
--]]
---@class LinearizedTreeDataProvider : TreeDataProvider
---@field linearized TreeNode[]?
local LinearizedTreeDataProviderMixin = CreateFromMixins(TreeDataProviderMixin);
...
---@param indexBegin integer | nil
---@param indexEnd integer | nil
---@param excludeCollapsed boolean
...
function ns:CreateTreeDataProvider()
   local dataProvider = CreateFromMixins(LinearizedTreeDataProviderMixin);
   dataProvider:Init();
   return dataProvider;
end

---@class FilterableTreeDataProvider : LinearizedTreeDataProvider
---@field filterPredicate fun(node:TreeNode):boolean
---@field filtered TreeNode[]?
local FilterableTreeDataProviderMixin = CreateFromMixins(LinearizedTreeDataProviderMixin);
...
function FilterableTreeDataProviderMixin:Invalidate()
   self.filtered = nil;
   LinearizedTreeDataProviderMixin.Invalidate(self);
end

function FilterableTreeDataProviderMixin:SetFilterPredicate(filterPredicate)
   self.filterPredicate = filterPredicate;
   self:Invalidate();
end

function FilterableTreeDataProviderMixin:HasFilterPredicate()
   return self.filterPredicate ~= nil;
end

function FilterableTreeDataProviderMixin:ClearFilterPredicate()
   self.filterPredicate = nil;
   self:Invalidate();
end


function FilterableTreeDataProviderMixin:Flush()
   self.filtered = nil;
...
function ns:CreateFilterableTreeDataProvider()
   local dataProvider = CreateFromMixins(FilterableTreeDataProviderMixin);
   dataProvider:Init();
   return dataProvider;
end
